...
import render from './gx-setup'
import utils from '@src/utils/utilsPlugin.js'
// ===
// Utility functions
// ===
...
const globalComponentFiles = walkSync(
  path.join(__dirname, '../../src/components'),
  []
).filter((fileName) => /Gx[A-Z]\w+\.(vue)$/.test(fileName))

for (const fileName of globalComponentFiles) {
  const componentName = fileName.match(/(components.+)(Gx.+)\.vue$/)[2]
  const componentConfig = require(path.join(
    __dirname,
    '../../src/',
    fileName.match(/(components.+)(Gx.+)\.vue$/)[1]
  ) + fileName.match(/(components.+)(Gx.+)\.vue$/)[2])
...
// A helper for mounting TestComponent with a wrapper using Vuelidate and Buefy.
